...
func hixie76ClientHandshake(config *Config, br *bufio.Reader, bw *bufio.Writer) (err error) {
	...
	if config.handshakeData != nil {
		...
		n, err := strconv.ParseUint(config.handshakeData["number1"], 10, 0)
		if err != nil {
			panic(err)
		}
		number1 = uint32(n)
		key2 = config.handshakeData["key2"]
		n, err = strconv.ParseUint(config.handshakeData["number2"], 10, 0)
		if err != nil {
			panic(err)
		}
		number2 = uint32(n)
...
func (c *hixie76ServerHandshaker) ReadHandshake(buf *bufio.Reader, req *http.Request,
	_ ProtocolChooser) (code int, err error) {
...
func (c *hixie75ServerHandshaker) ReadHandshake(buf *bufio.Reader, req *http.Request,
	_ ProtocolChooser) (code int, err error) {
