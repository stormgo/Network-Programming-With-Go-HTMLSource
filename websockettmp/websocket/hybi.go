...
var (
	...
	ErrUnsupportedSubProtocol = &ProtocolError{"unsupported subprotocol"}
)
...
func hybiClientHandshake(config *Config, br *bufio.Reader, bw *bufio.Writer) (err error) {
	...
	if offeredProtocol != "" {
		matched := false
		for i := 0; i < len(config.Protocol) && !matched; i++ {
			matched = (config.Protocol[i] == offeredProtocol)
		}
		if !matched {
...
func (c *hybiServerHandshaker) ReadHandshake(buf *bufio.Reader, req *http.Request,
	...
	if protocol != "" {
		// Negotiate subprotocol between client and server
		if protocolChooser == nil {
			return http.StatusBadRequest, ErrBadWebSocketProtocol
		}
		protocols := strings.Split(protocol, ",")
		for n, str := range protocols {
			protocols[n] = strings.TrimSpace(str)
		}
		preferredProtocol, err := protocolChooser(protocols)
		if err != nil {
			return http.StatusBadRequest, err
		}
		if preferredProtocol != "" {
			// Check: it's got to be in the protocols array
			var found bool
			for n := 0; n < len(protocols) && !found; n++ {
				found = (preferredProtocol == protocols[n])
			}
			if !found {
				return http.StatusBadRequest, ErrUnsupportedSubProtocol
			}
		}
		c.Protocol = []string{preferredProtocol}
	}
...
func (c *hybiServerHandshaker) AcceptHandshake(buf *bufio.Writer) (err error) {
	...
	if len(c.Protocol) == 1 && c.Protocol[0] != "" {
