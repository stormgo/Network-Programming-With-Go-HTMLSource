...
`))
	...
	code, err := handshaker.ReadHandshake(br, req,
		SimpleChooser([]string{"superchat"}))
	if err != nil {
		t.Errorf("handshake failed: %v", err)
	}
...
func TestHybiErrSubProtocol(t *testing.T) {
	config := new(Config)
	handshaker := &hybiServerHandshaker{Config: config}
	br := bufio.NewReader(strings.NewReader(`GET /chat HTTP/1.1
Host: server.example.com
...
`))
	...
	_, err = handshaker.ReadHandshake(br, req,
		func([]string) (string, error) {
			return "", ErrUnsupportedSubProtocol
		})
	if err == nil {
		t.Errorf("expected subprotocol error")
	}
}

func TestHybiNoSuitableSubProtocol(t *testing.T) {
	config := new(Config)
	handshaker := &hybiServerHandshaker{Config: config}
	br := bufio.NewReader(strings.NewReader(`GET /chat HTTP/1.1
Host: server.example.com
...
`))
	...
	code, err := handshaker.ReadHandshake(br, req,
		SimpleChooser([]string{"none-of-them"}))
	if err != nil {
		t.Errorf("handshake failed: %v", err)
	}
	if code != http.StatusSwitchingProtocols {
